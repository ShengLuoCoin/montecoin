...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 365 * 1;	// stake age of full weight: 1 year
unsigned int nStakeMinL2Age = 60 * 60 * 24 * 15;	// 15days,got 5% rates not use in

//block targetspacing
unsigned int nStakeTargetSpacing = 60 * 2;			// 60 sec block spacing
unsigned int nWorkTargetSpacing  = 60 * 2;			// 60  sec block spacing

static const int64 nStakeTargetTimespan = 60 * 60 * 5; // 5 hour
static const int64 nWorkTargetTimespan =  60 * 60 * 5; // 5 hour

int64 nChainStartTime = 1434556800;//2015.6.18
...
const string strMessageMagic = "Montecoin Signed Message:\n";
...
//montecoin:for explorer home page latest txs
...
int64 GetProofOfWorkReward(int nPOWHeight, int64 nFees)
{
...
int64 GetProofOfStakeReward(int64 nCoinAge,int64 nCoinAgeL1,int64 nCoinAgeL2, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    if(pindexBest->nMoneySupply < MAX_MONEY)
    {
        nSubsidy = (nCoinAge * MINT_PROOF_OF_STAKE_L1) / 365;
    }
...
unsigned int GetNextTargetRequiredV2(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    while(pindexPrevPrev  && pindexPrevPrev->pprev && (nActualInterval < 120))
    {
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Montecoin
    bool fStrictPayToScriptHash = true; // Always active in Montecoin
    ...
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;//TCTODO
    ...
    {
        if (vtx[0].GetValueOut() > (GetProofOfWorkReward(pindex->nPOWHeight, nFees)/*+GetProofOfManpowerReward(pindex->nHeight)*/))
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Montecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "We firmly believe digital currency will change the future change the world--18/6/2015";
       ...
       // block.print();
        ...
        assert(block.hashMerkleRoot == uint256("0x0847eb5dc0542ff5c48b250146d6fd4c6408167c3ed6ceee35d037341b6c5076"));
...
unsigned char pchMessageStart[4] = { 0x3f, 0xfb, 0xfb, 0xd5 };
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nFees = 0;
    {
        ...
        {
            pblock->vtx[0].vout[0].nValue = GetProofOfWorkReward(pindexPrev->nPOWHeight+1, nFees);
        }
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    //upgradetodo
    while(pindexBest && pindexBest!=pindexGenesisBlock && (GetTime() > (pindexBest->GetBlockTime() + 600)))
    {
        if (fShutdown)
            return;
        Sleep(10000); //10 s
