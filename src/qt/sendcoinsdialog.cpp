    ...
    model(0)
{
    ...
    ui->sendButton->setIcon(QIcon());
     ...
     ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Montecoin address"));
#endif
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        QMessageBox::warning(this, tr("区块链同步中"),
            tr("区块链同步结束之后才能进行挖矿、铸币、发币等操作，请耐心等待"),
  ...
  //  fWalletUnlockMintOnly = false; //后来测试自己修改过来的
 //   emit sendCoinsClicked("Lock Wallet");
    ...
    for(int i = 0; i < ui->entries->count(); ++i)
    {
        ...
        if(entry)
        {
            ...
            if(entry->isDeposit(nDepositID))
            {
                if(entry->isDepositIDEmpry())
                {
                    QMessageBox::warning(this, tr("Game Deposit"),
                        tr("Please enter you MozzliveID"),
                        QMessageBox::Ok, QMessageBox::Ok);
                    return;
                }
                fDeposit = true;
    ...
    QString strID ="<html>"+tr("")+"</html>""<html>"+"<br>"+tr("")+"</html>"
            "<html>"+"<br>"+tr("This is a cucc transacion")+"</html>"
                    "<html>"+"<br>"+tr("Your MozzliveID: ")+"</html>"
                    +QString::number(nDepositID);
    QString empty;

    fNewRecipientAllowed = false;

    QMessageBox::StandardButton retval = QMessageBox::question(this, tr("Confirm send coins"),
                          tr("Are you sure you want to send %1?").arg(formatted.join(tr(" and ")))+ (fDeposit?strID:empty),
...
//    WalletModel::SendCoinsReturn sendstatus = model->sendCoins(recipients);
    ...
    {
        if(fDeposit)
             sendstatus = model->sendCoins(recipients,true,nDepositID);
        else
    ...
    {
       if(fDeposit)
           sendstatus = model->sendCoins(recipients, true,nDepositID,CoinControlDialog::coinControl);
       else
    ...
    case WalletModel::InValidDepositTX:
        QMessageBox::warning(this, tr("Send Coins"),
            tr("Error: Deposit Transaction creation failed."),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
     ...
     if (model)
     {
        ...
        if (text.isEmpty())
         ...
         else if (!CBitcoinAddress(text.toStdString()).IsValid())
         {
             ...
             ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Montecoin address"));
         }
         else
         {
